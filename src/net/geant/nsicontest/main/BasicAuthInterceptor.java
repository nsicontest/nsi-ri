...
/**
 * Provides basic-auth for web service requests
 * 
 * Original receipt can be found here: http://chrisdail.com/2008/03/31/apache-cxf-with-http-basic-authentication/
 */
public class BasicAuthInterceptor extends SoapHeaderInterceptor {
	
	private static Logger log = Logger.getLogger(BasicAuthInterceptor.class);
    ...
    public void handleMessage(Message message) throws Fault {
        ...
        log.debug("Logging in user: " + policy.getUserName());        
        // Verify the password
        String realPassword = users.get(policy.getUserName());
        if (realPassword == null || !realPassword.equals(policy.getPassword())) {
            log.debug("Invalid username or password for user: " + policy.getUserName());
